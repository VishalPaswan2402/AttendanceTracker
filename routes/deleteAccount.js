...
router.delete("/Delete-My-Account",wrapAsync(async(req,res,next)=>{
    ...
    let currTech=await Teacher.findById(techId);
    if(currTech.username===username && currTech.teacherName===tName && currTech.teacherEmail===tEmail && currTech.collegeName===collegeName){
        let hasPass=await bcrypt.compare(password,currTech.password);
        if(hasPass===true){
            ...
            req.flash("error","Please enter correct password to delete your account.");
